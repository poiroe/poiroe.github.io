...
async function fetchImagesFromGitHub() {
  try {
    const response = await fetch(`https://raw.gitmirror.com/poiroe/poiroe.github.io/main/PackeryPW/list/vrchat.json`);
    const data = await response.json();
    
    const imageUrls = data.map(file => {
      const fileName = encodeURIComponent(file.name);
      return `https://raw.gitmirror.com/gh/poiroe/picx-images-hosting/master/${folderPath}/${fileName}`;
    });
